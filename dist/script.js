...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_filter__WEBPACK_IMPORTED_MODULE_8__["default"])('.portfolio-menu', '.portfolio-block', '.portfolio-no');
...
var accordion = function accordion(triggetsSelector
/*, itemsSelector */
) {
  var btns = document.querySelectorAll(triggetsSelector); // ! Вывод акордиона через JS+CSS
...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  burgerElem.addEventListener('click', function () {
    if (menuElem.style.display == 'none' && window.screen.availHeight < 993) {
  ...
  window.addEventListener('resize', function () {
    if (window.screen.availHeight < 993) {
      menuElem.style.display = 'none';
    }
  });
};
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var sum = 0; // ПОЛУЧАЕМ ДАННЫЕ ИЗ ФАЙЛА opt.json

  Object(_services_requests__WEBPACK_IMPORTED_MODULE_2__["getResource"])('assets/opt.json') // * при работе с файлом
  .then(function (res) {
    function createOptions(response, block) {
      response.forEach(function (_ref) {
        ...
        title ? select.title = title : select.title = '';
        block.appendChild(select);
      });
    }

    createOptions(res.size, sizeBlock);
    createOptions(res.material, materialBlock);
    createOptions(res.options, optionsBlock);
  }).catch(function (error) {
    return console.log(error);
  });

  var calcFunc = function calcFunc(response) {
...
var changeModalState = function changeModalState(state) {
  var size = document.querySelectorAll('#size'),
      ...
      promocode = document.querySelectorAll('.promocode'); // функция для добавления данных с всех форм в переменную changeModal
        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              i === 0 ? state[prop] = 'Холодное' : state[prop] = 'Тёплое'; //очищать все чекбоксы кроме чекнутого
...
var drop = function drop() {
  var fileInputs = document.querySelectorAll('[name="upload"]'); // создвем МАССИВ событий для addEventListener перебираем и даем поведение
  ...
  function unhighlight(item) {
    item.closest('.file_upload').style.border = 'none'; // подгоняем цвет под рсзные формы
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      input.files = e.dataTransfer.files; // fetch.input.files // надо отправить ФАЙЛ НА СЕРВЕР. не реализовано
      // коррекция имени в выводе

      var dots; // пемеренная будет содержать '...' или вообще ничего
...
var filter = function filter(filterMenu, filterBlock, filterNo) {
      ...
      if (!targetClass.contains('active')) {
        images.forEach(function (image) {
          ...
          if (image.classList.contains(targetClass.value)) {
            ...
            no.classList.remove('animated', 'fadeIn'); // * записываем в переменную на каждой итерации цикла
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      e.preventDefault(); // создаем блок с сообщением

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.parentNode.appendChild(statusMessage); // всплытие блока формы на место после отправки
      ...
      statusMessage.appendChild(textMessage); // собираем данные, отправляем в formData
      ...
      var api;
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question; //написание запроса и отправка на сервер. Промис
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    elem.focus(); // установить курсор в нужное место в инпуте

    if (elem.setSelectionRange) {
      elem.setSelectionRange(pos, pos);
    } else if (elem.setSelectionRange) {
      var range = elem.setSelectionRange;
      range.collapse(true);
  ...
  function createMask(e) {
    ...
    var matrix = '+7 (___) ___ __ __',
        ...
        val = this.value.replace(/\D/g, ''); // если пользователь удаляет +7 то цифры подставляются назад
...
var modals = function modals() {
  var btnPreaaed = false; // * фиксируем, кликнул ли пользователь кудато false 
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        windows.forEach(function (item) {
          item.style.display = 'none'; // добавляем анимацию из библиотеки animated.css
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display; // если модальное окно уже открыто, не открывать его по таймауту

      document.querySelectorAll('[data-modal]').forEach(function (item) {
        if (getComputedStyle(item).display !== 'none') {
          //getComputedStyle - не инлайновый а реальные стили обьекта
          display = 'block';
        }
      });
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      ...
      var scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); // если расстояние сверху + расстояние  видимого экрана >= высоты скрола
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    // getResource('http://localhost:3000/styles') // * при работе с сервером.
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('assets/db.json') // * при работе с файлом
    .then(function (res) {
      return createCards(res.styles);
    }) // работа с сервером, удалить styles
    .catch(function (error) {
      return console.log(error);
    });
    this.remove();
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 1,
      paused = false;
  var items = document.querySelectorAll(slides); // замыкание слайдов. Если доходит до конца или до начала
